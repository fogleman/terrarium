...
func intersectSegment(z float64, v0, v1 vector) (vector, bool) {
	if v0.Z == v1.Z {
		return vector{}, false
	}
	t := (z - v0.Z) / (v1.Z - v0.Z)
...
func intersectTriangle(z float64, pt1, pt2, pt3 vector) (vector, vector, bool) {
	...
	n := vector{p1.Y - p2.Y, p2.X - p1.X, 0}
	e1 := pt2.Sub(pt1)
	e2 := pt3.Sub(pt1)
...
func slice(grid []float64, w, h int, z float64) []pair {
	...
	for y := 0; y < h-1; y++ {
		...
		for x := 0; x < w-1; x++ {
			...
			z4 := (z0 + z1 + z2 + z3) / 4
			v0 := vector{fx, fy, z0}
			v1 := vector{fx + 1, fy, z1}
			v2 := vector{fx, fy + 1, z2}
			v3 := vector{fx + 1, fy + 1, z3}
			v4 := vector{fx + 0.5, fy + 0.5, z4}
